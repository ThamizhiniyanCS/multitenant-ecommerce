  ...
  getOne: baseProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const product = await ctx.payload.findByID({
        collection: 'products',
        id: input.id,
        depth: 2, // NOTE: Load the "product.image", "product.tenant" and "product.tenant.image"
        select: {
          content: false,
        },
      })
  ...
  getMany: baseProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input.tenantSlug) {
        ...
        // NOTE: If we are loading products for public storefront (no tenantSlug)
      ...
      const data = await ctx.payload.find({
        collection: 'products',
        depth: 2, // NOTE: Populate "category", "image", "tenant"
        where,
        sort,
        page: input.cursor,
        limit: input.limit,
