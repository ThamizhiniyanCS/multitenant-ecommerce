...
export const checkoutRouter = createTRPCRouter({
  verify: protectedProcedure.mutation(async ({ ctx }) => {
    const user = await ctx.payload.findByID({
      ...
      depth: 0, // NOTE: user.tenants[0].tenant is going to be a string (tenant ID)
    })
    ...
    const tenantId = user.tenants?.[0]?.tenant as string // NOTE: This is an ID because of `depth: 0`
